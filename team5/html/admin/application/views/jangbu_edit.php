...
?>
 <script>
	/*$(window).load(function () {
		...
		var days = form1.days.value;

		for(var i=0; i<days; i++){  
			if(inDate.getDay() == 0 || inDate.getDay() == 1){//0:금, 1:토 / 주말가격 적용
				charge += form1.roomPrice.value * 0.3
	...
	$(function(){
		...
		for(var i=0; i<days; i++){  
			if(inDate.getDay() == 1 || inDate.getDay() == 2){//1:금, 2:토 / 주말가격 적용
				form1.roomCharge.value = Number(form1.roomCharge.value) + 1;	
			}
			inDate.setDate(inDate.getDate()+1);
		}
				...
				<div class="row">
                    ...
                    <div class="card shadow mr-4 col" style="padding:0px" >
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">예약수정</h6>
                        </div>
                        <div class="card-body">
							<form name="form1" method="post" enctype="multipart/form-data" class="user">
							  <div class="row">
								  <div class="form-group col">
									...
									<div class="row">
										...
										<input type="text" class="form-control form-control-user mx-2 col-8" name="room_name" value="<?=$row->room_name?>" placeholder="예약객실" readonly>
								  ...
								  <div class="form-group col">
									<label>예약자</label>
									<div class="row">
										<input type="hidden" value="<?=$row->member_no?>" name="member_no">
										<input type="text" value="<?=$row->member_name?>" class="form-control form-control-user mx-2 col-8" name="member_name" id="product" placeholder="예약자" readonly>
							 ...
							 <div class="row">
								  <div class="form-group col">
									<label>체크인</label>
									<div class="row">
										<div class="form-group  input-group date col" value="<?=$row->inday?>"  id="inday">
												<input type="text" value="<?=$row->inday?>" name="txtInday"id="txtInday" value=""class="form-control form-control-user">
									  ...
									  <div class="form-group col">
										...
										<div class="row">
											<div class="form-group  input-group date col" id="exitday">
												<input type="text" value="<?=$row->exitday?>" name="txtExitday" id="txtExitday" value=""class="form-control form-control-user" >
												<div class="input-group-append">
													<div class="input-group-text">
														<div class="input-group-addon"><i class="far fa-calendar-alt fa-lg"></i></div>
													</div>
								...
								<div class="row">
								 <?php
									...
									foreach($selectedDetail as $row1){
										$optionDetail=$optionDetail."_".$row1->detail_no;
										$roomDetailName=$roomDetailName.$row1->detail_name."[".$row1->detail_price."],";
										$price = $price + $row1->detail_price;
								
									}
									$roomDetailName=substr($roomDetailName,0,-1);
									$roomPrice = $price + $row->room_price;
									$roomPriceDetail = $row->room_price." + ". $price." = ".$roomPrice
								?>
								  <div class="form-group col">
									<label>선택 옵션</label>
									<input type="text" class="form-control form-control-user mx-2 col-12" name="roomDetailName" 
									value ="<?=$roomDetailName?>" 
									placeholder="선택 옵션" onKeyUp="cal_totalPrices();" readonly>
									</div>
								...
								<div class="row">
								 <div class="form-group col">
										...
										<input type="text" class="form-control form-control-user mx-2 col-12"  name="roomPriceDetail"  value ="<?=$roomPriceDetail?>" placeholder="단가" readonly>
											...
											<input type="hidden" name="optionPrice"  value ="<?=$price?>">
											<input type="hidden" name="roomPrice"  value ="<?=$roomPrice?>">
											<input type="hidden" name="optionDetail"  value ="<?=$optionDetail?>">
										
								  </div>
								  <div class="form-group col">
									<label>총 숙박일</label>
									<input type="text"value="<?=$row->days?>"class="form-control form-control-user mx-2 col-12" name="days" placeholder="숙박일" onKeyUp="cal_totalPrices();" readonly>
									</div>
							  ...
							  <div class="row">
								  <div class="form-group col">
										...
										<label>금액(금,토는 20% 할인)</label>
											...
											<input type="text"value="<?=$row->price?>"class="form-control form-control-user mx-2 col-12" name="totalPrice"  placeholder="금액" readonly>
										</div>
