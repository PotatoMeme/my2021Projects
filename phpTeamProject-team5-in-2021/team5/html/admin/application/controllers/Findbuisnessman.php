<?
	class Findbuisnessman extends CI_Controller{ //클래스이름 첫 글자는 대문자
		
		function __construct()
		{
			parent::__construct();
			$this->load->database();            // 데이터베이스 연결
            $this->load->model("findbuisnessman_m");    // 모델 Member_m 연결
		...
		public function lists()
        {
			...
			if($text1=="")
				$base_url="/findbuisnessman/lists/page";
			else
				$base_url="/findbuisnessman/lists/text1/$text1/page";

			$page_segment = substr_count( substr($base_url,0,strpos($base_url,"page")) , "/" )+1;
			$base_url = "/~team5/admin" . $base_url;

			$config["per_page"]	 = 5;									// 페이지당 표시할 line 수
			$config["total_rows"] = $this->findbuisnessman_m->rowcount($text1);  // 전체 레코드개수 구하기
			...
			$data["list"]=$this->findbuisnessman_m->getlist($text1,$start,$limit);   // 해당페이지 자료읽기
			$data["text1"]=$text1;								//text1전달을 위한 처리  

            $this->load->view("main_header_nomenu");                   // 상단출력(메뉴)
            $this->load->view("findbuisnessman_list",$data);				// member_list에 자료전달
