<?
	...
	class Jangbu extends CI_Controller{
		...
		public function lists()
        {
			...
			$data["list"]=$this->Jangbu_m->getlist($text1,$text2,$text3,$text4,$start,$limit);//지정한 페이지만큼 읽어옴
		...
		public function view()
		{
			...
			$data["text3"]=$text3;
			$data["text4"]=$text4;
			$data["row"] = $this->Jangbu_m->getrow($no,$text1,$text2,$text3,$text4);
		...
		public function add()
		{
			...
			{
				$data["inday"]=date("Y-m-d",strtotime("-1 day"));
				$this->load->view("main_header");
				$this->load->view("jangbu_add",$data);     // 입력화면 표시
				$this->load->view("main_footer");				$this->load->view("main_footer");
		...
		public function edit()
		{
			...
			$this->form_validation->set_rules("room_no","객실","required|numeric");
			$this->form_validation->set_rules("member_no","고객","required|numeric");
			$this->form_validation->set_rules("detail_no","옵션","required|numeric");
			$this->form_validation->set_rules("inday","체크인","required");
			$this->form_validation->set_rules("exitday","체크아웃","required");
			$this->form_validation->set_rules("days","총숙박일","required|numeric");
			$this->form_validation->set_rules("price","금액","required|numeric");

			if ($this->form_validation->run()==FALSE) // 수정버튼을 클릭한 경우(값 없이 호출된 경우)
			{
				$data["no"] = $page;
		        $this->load->view("main_header");
				$data["list"]=$this->Jangbu_m->getrow($no);
			...
			$this->Jangbu_m->updaterow($data,$no);
			redirect("/~team5/jangbu/lists/".$text1.$text2.$text3.$text4.$page);
			}
		}
